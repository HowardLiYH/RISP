"""
Leave-one-reactivation-out (LORO) analysis for the E1f-L3 headline deficit.

Reviewer objection addressed: is the L3 walk-forward forgetting deficit
Gamma = post_react(A1-monolith-erm) - post_react(A9-oracle-pinned)
carried by a single reactivation event (e.g. the 2020 COVID reactivation)?

Protocol: reruns ONLY the four relevant walk-forward arms with EXACTLY the
same config and seeding as e_french_L3.py (rng 1311*s+17 per arm, 20 seeds,
K=2, hard memory, probe=15, min_dormancy=90, k=5, w_max=0.2, dd_thresh=0.15
via label_L3 defaults). The only addition is collect_react=True, which is
purely observational. A hard sanity check asserts the recomputed aggregate
post_react per seed matches results/e_french49_L3_dissoc.json walkforward
raw values, proving the instrumentation changed nothing.

No tuning, no selection: every qualifying reactivation is reported, and the
pre-specified exclusions (2020; 2008-2010; 2022; per-decade eras) are fixed
in this file before it was first run.

Output: ../results/e_french49_L3_loro.json
"""
...
def main():
    ...
    for r in inventory:
        print(f"  #{r['i']:2d} {r['date']} regime={r['regime']} "
              f"dormancy={r['dormancy']:4d}d days={r['n_probe_days_counted']}",
              flush=True)

    # ---- rerun the four arms, identical seeding to e_french_L3.py ----
    ...
    ref = json.load(open(RESULTS / "e_french49_L3_dissoc.json"))
    wf = ref["walkforward"]
    sanity = {}
    for a in LORO_ARMS:
    ...
    out = {"config": {"seeds": SEEDS, "K": 2, "memory": "hard",
                      ...
                      "k": K_SEL, "w_max": W_MAX, "dd_thresh": 0.15,
    ...
    for cname in CONTRASTS:
        ...
        # pre-specified calendar exclusions
        ...
        for k, v in exclusions.items():
            print(f"  {k}: {v['mean']:.5f}±{v['ci95']:.5f} "
                  f"(n_react={v['n_react']})", flush=True)
        for era, v in eras.items():
            print(f"  era {era}: {v['mean']:.5f}±{v['ci95']:.5f} "
                  f"(n_react={v['n_react']})", flush=True)

    with open(RESULTS / "e_french49_L3_loro.json", "w") as fh:
        json.dump(out, fh, indent=1, default=float)
    print("\nLORO ANALYSIS COMPLETE ->", RESULTS / "e_french49_L3_loro.json",
          flush=True)
