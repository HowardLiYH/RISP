...
and daily U.S. industry portfolios 1926--2025, where $\G$'s sign predicted
...
manuscript \cite{companion2026} \todo{arxiv-id}; this paper is the
...
Where $\G$ is positive, retention---however implemented---collects it;
...
per-regime adapter store, an expert library with pinned members. The
...
full ordering (joint Holm $5.2{\times}10^{-65}$); cost-amplified \\
...
ordering emerges (joint Holm $5.2{\times}10^{-65}$), with the retaining
...
miss (12\%: $\G$ marginally negative, arm table flat) is scored against
...
prices the \emph{parameter} claim, and the turnover product is
separately purchasable---a no-trade hysteresis band on the deployed
baseline is the obvious desk countermeasure, and a banded-baseline
control battery is lodged as a forward registration with its adverse
branch stated (if banding alone closes the net gap where $\G \le 0$,
the net-cost sentence above is relabeled as buyable discipline, at
full prominence). One sensitivity is
...
negative) and \textbf{12/12 under the weak form} (dissociation iff
$\G$ significantly positive). The four strong-form misses share one
...
diagnostics frozen on data through \textbf{2026-06-30}
