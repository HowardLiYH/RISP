...
Neither constant is load-bearing: a lodged sensitivity battery varying
...
collected 73\% of the deficit (residual versus the protected twin
$+2.5 \pm 2.5$\,bps, not significant at 20 seeds), behaving as what it
is---data retention without parameter isolation. A9 represents every
...
record's deep-dormancy range. Retention pays below the number; a
...
avoidable regret per reactivation is ${\sim}165$--$171$\,bps-days
whether measured at $N_p{=}15$ or $N_p{=}30$. The rule's two-sided honesty is
...
over seeds, conditional on each window's single history). The
...
$\G$ with a flat arm table. The two 1990--2025 walk-forward tests that
...
\begin{enumerate}
  ...
  not a vibe: whenever a new qualifying reactivation enters the
