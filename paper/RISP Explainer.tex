...
carrying cost, $D \lesssim 150$ days; above $\approx 25\%$, \emph{no}
...
$=$ shuffled $=$ pooled). The ten-arm experiment on
regime-stitched real crypto (all arms except the oracle-INV skyline) is
consequently \emph{flat} (every pairwise $p > 0.2$ in the pre-registered
test family; the numerically best arm is the \emph{non-learning}
Hedge-over-fixed-strategies baseline)---exactly what the failed gate
predicts, demonstrated rather
